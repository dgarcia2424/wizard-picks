...
def predict_games(date_str: str) -> pd.DataFrame:
    ...
    for _, g in lineups.iterrows():
        ...
        if expected_cols != list(feat_df.columns):
            print(f"[WARN] NRFI scorer: feature list mismatch for {home}@{away} — cols may have drifted")
        nan_cols = feat_df.columns[feat_df.isnull().any()].tolist()
        if nan_cols:
            print(f"[WARN] NRFI scorer: NaNs after reindex for {home}@{away} in {nan_cols} — filling 0")
